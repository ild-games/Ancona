...
class CameraComponent
{
  public:
     ...
     */
    CameraComponent(const View &originalView, float renderPriority, DrawableSystem *drawableSystem, float scale = 1.0f,
                    Vector2f offset = Vector2f(0, 0),
                    Vector2f lowerBounds = Vector2f(std::numeric_limits<float>::min(),
                                                    std::numeric_limits<float>::min()),
                    Vector2f upperBounds = Vector2f(std::numeric_limits<float>::max(),
    ...
    void lowerBounds(const Vector2f &lowerBounds)
    {
        _lowerBounds = lowerBounds;
    }
    const Vector2f &lowerBounds()
    {
        return _lowerBounds;
    }
    void upperBounds(const Vector2f &upperBounds)
    {
        _upperBounds = upperBounds;
    }
    const Vector2f &upperBounds()
    {
        return _upperBounds;
    }
    const View &view()
...
class CameraSystem : public UnorderedSystem<CameraComponent>
{
  public:
     ...
     */
    CameraSystem(std::string name, SystemManager &manager);
     ...
     */
    CameraComponent *CreateComponent(const Entity &entity, const View &originalView, float renderPriority,
                                     DrawableSystem *drawableSystem, float scale = 1.0f);
};
} // namespace ild
