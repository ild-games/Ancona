#ifndef Ancona_Platformer_Action_Action_hpp
#define Ancona_Platformer_Action_Action_hpp
...
/**
 * @brief An action is used to interact with the position system. (UNIT TESTED)
 * @author Jeff Swenson
 */
template <class SubClass> class Action
{
  public:
    /**
     ...
     */
    SubClass *priority(int priority)
    {
        _priority = priority;
     ...
     *
     * @param duration duration of the action.  An action with a duration of 0 will expire after a single update step.
     * The duration is the number of seconds the action should apply for.
     *
     * @return A pointer to the action.
     */
    SubClass *duration(float duration)
    {
        _duration = duration;
        return static_cast<SubClass *>(this);
    }
    ...
    /**
     * @brief Check to see if an action has run for its entire duration. If an action is done then no other methods can
     * be called on it.
     *
