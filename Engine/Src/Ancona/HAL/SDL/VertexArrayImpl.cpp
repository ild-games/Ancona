...
sf::VertexArray & priv::VertexArrayImpl::sfmlVertexArray() const
{
    return static_cast<sf::VertexArray &>(*_sfmlDrawable);
}
...
{
    _pimpl = std::make_unique<priv::VertexArrayImpl>((sf::PrimitiveType) type, vertexCount);
}
