...
void JumpyKeyboard::HandleInput()
{
    ...
    {
        Entity player = _systems->GetSystemManager().GetEntity("player");
        _systems->GetFadeDrawable().CreateComponent(
                player,
                *_systems->GetDrawable()[player],
                false);
    }
}
    ...
    _physics(physicsComponent)
{
    ...
    jumpAction->Duration(ActionDuration::PERSISTENT);
}
